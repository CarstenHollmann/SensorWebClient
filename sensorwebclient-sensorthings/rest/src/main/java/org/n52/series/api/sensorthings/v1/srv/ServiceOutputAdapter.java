...
import static org.n52.series.api.sensorthings.v1.srv.QueryParameterAdapter.createQueryParameters;
...
import org.n52.series.api.sensorthings.v1.io.ServiceConverter;
import org.n52.shared.requests.query.QueryParameters;
import org.n52.shared.serializable.pojos.sensorthings.SensorThingsMetadata;

public class ServiceOutputAdapter extends AbstractOutputAdapter implements ServiceParameterService {

	@Override
	public boolean isKnownTimeseries(String timeseriesId) {
		for (SensorThingsMetadata metadatas : getSensorThingsMetadatas()) {
	...
	public ServiceOutput[] getExpandedParameters(IoParameters map) {
		...
		for (SensorThingsMetadata metadata : getSensorThingsMetadatas()) {
	...
	public ServiceOutput[] getCondensedParameters(IoParameters map) {
		...
		List<ServiceOutput> allServices = new ArrayList<ServiceOutput>();
		for (SensorThingsMetadata metadata : getSensorThingsMetadatas()) {
			if (matchesQuery(query, (SensorThingsMetadata) metadata)) {
				ServiceConverter converter = new ServiceConverter((SensorThingsMetadata) metadata);
				allServices.add(converter.convertCondensed((SensorThingsMetadata) metadata));
			}
		}
		return allServices.toArray(new ServiceOutput[0]);
	}

	private boolean matchesQuery(QueryParameters query,
			SensorThingsMetadata metadata) {
